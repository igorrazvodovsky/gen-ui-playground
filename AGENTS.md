...
- React integration: `Renderer` consumes `componentRegistry` from `components/ui/index.ts`. `DataProvider`, `VisibilityProvider`, and `ActionProvider` wrap `DashboardPage`; add `ValidationProvider` if you introduce custom validators and register them in the catalog’s `functions`.
